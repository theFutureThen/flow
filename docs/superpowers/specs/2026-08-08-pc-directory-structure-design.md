...
| --- | --- | --- | --- |
| 运行环境 | preload 全 Node 权限 | Node 子进程 + 独立 v8 isolate + RPC | L0 解释执行 / L1 沙箱 / L2 受控子进程（§7） |
| 能力边界 | 无技术限制 | 官方称"没有进一步的沙箱隔离"，可访问文件系统与网络 | L0 无代码执行，L1 白名单 API，L2 受控 Node |
| 防恶意手段 | 源码强制可读 + 审查 | 强制开源 + 人工审查 + CI 校验 | L0 无需，L1 技术隔离，L2 隔离 + 审查 |
| 生态规模 | 3000+ 插件，500 万用户 | 大 | — |

**关键结论：两个头部产品都依赖审查而非技术隔离。** 纯沙箱方案在安全性上优于两者，但生态能力天花板显著更低——uTools 现有插件中相当一部分在纯沙箱内无法实现。本设计据此引入分层插件模型（§7）。

### 2.3 WorkBuddy 的启发

腾讯 [WorkBuddy](https://ai-bot.cn/sites/73160.html) 是 AI 原生桌面智能体，与 launcher 不是同一品类（"描述需求 → AI 规划拆解 → 自动执行"，而非"敲指令 → 打开工具"），但其扩展体系有两点直接可借鉴：

1. **Skill 就是一个 Markdown 文件**，用户自己写或让 AI 生成，零门槛。对照本设计，L1 沙箱插件虽免审，仍要求会写 JS、要打包、要走安装流程；而 uTools 现有插件中相当一部分本质只是"一个 URL 模板"或"一段文本转换规则"，为其写 JS 是过度工程。这直接催生了 §7 的 L0 层。
2. **能力层用 MCP 标准描述**。uTools 亦有 `tools` 字段以 JSON Schema 向 AI Agent 暴露插件能力。二者指向同一个杠杆：**一份能力声明，既供 launcher 检索调用，也供 AI Agent 调用**，无需维护两套。对面向全球的产品这一点尤其成立——MCP 是国际标准，自定义协议只能自己玩。

> 信息来源说明：WorkBuddy 官网为 SPA，未能抓取到实质内容，以上结论来自第三方评测与技术文章，可靠性低于 uTools 与 Raycast（后两者读的是官方开发者文档）。涉及具体机制的部分应在实施前实测验证。
...
| 插件模型 | 三层：L0 声明式零代码免审，L1 沙箱插件免审即时发布，L2 特权插件强制开源 + 审核 + 签名 |
| 能力声明 | 结构与 MCP tool schema 对齐（仅协议预留，不做实现，见 §7.6） |
...
**L2 特权插件确实触碰这条基线**——它需要在受控子进程中执行外部 JS。处理方式见 §7.4：L2 不放宽 Fuses，而是通过独立 `utilityProcess` 承载，宿主主进程自身的加载路径不变。
...
├── tier.ts            # L0 / L1 / L2 层级声明与各层可用能力集
├── capability.ts      # 能力声明，结构与 MCP tool schema 对齐（§7.6）
├── declarative.ts     # L0 声明式扩展的 schema（§7.3）
├── messages/
...
└── src/
    ...
    └── renderer/
        ...
        │   ├── settings/
        │   ├── declarative-editor/ # L0 扩展的创建与编辑界面
        │   ├── plugin-manager/
        │   └── plugin-host/        # 插件视图容器
...
1. 插件在 manifest 的 `commands` 中静态声明（L0 / L1 / L2 同构，registry 不区分层级）
2. 特权内置工具在 `main/features/index.ts` 中静态注册
3. 宿主自身动作（打开设置、切换主题、退出等）

指令的**触发方式**目前只有一种：用户唤起 launcher 后检索执行。结构上为非交互触发（定时、事件）留出位置，但第一版不实现——WorkBuddy 的定时任务说明这条路径有真实需求，但它属于自动化产品的范畴，与课题要求的 launcher 形态不同。
...
另一头，L1 虽然免审，仍要求开发者会写 JS、要打包、要走安装流程。而 uTools 现有插件中相当一部分本质只是一个 URL 模板或一段文本替换规则，为其写 JS 是过度工程，也把普通用户挡在了创作之外。

三层是这两难的解法：**按能力需求分流，让绝大多数扩展走无需审查甚至无需代码的路径，只有少数确实需要系统能力的插件承担审核成本。**

### 7.2 三层的边界

| | L0 声明式 | L1 沙箱插件 | L2 特权插件 |
| --- | --- | --- | --- |
| 形态 | 纯声明，无代码 | JS + 沙箱 | JS + 受控 Node |
| 运行环境 | 宿主解释执行 | 沙箱渲染上下文，无 Node | 独立 `utilityProcess` |
| 能力 | 参数替换、URL 跳转、文本规则、编排已有能力 | 宿主白名单 API | 白名单 API + 受限文件系统 + 受控网络 |
| 发布 | 免审，可一键分享 | 免审，即时上架 | 强制开源 + 人工审核 + 签名 |
| 创建方式 | 用户在设置界面内创建 | 开发者编写 | 开发者编写 + 提交审核 |
| 适用 | 自定义搜索、URL 模板、文本转换 | 计算、转换、纯 UI | 本地文件批处理、本地服务、系统级交互 |
| 默认 | 否 | 是——未显式声明层级的插件按 L1 处理 | 否 |

层级在 manifest 中显式声明，由 `protocol/tier.ts` 定义各层能力集。**能力集是白名单而非黑名单**：新增能力必须显式加入某层，遗漏时默认不可用，而不是默认可用。

### 7.3 L0 的价值在生态冷启动

L0 的意义不在技术，在于**新平台最大的问题是没有插件**。L0 让普通用户在没有开发者参与的情况下自己造指令并一键分享，绕开了"没插件 → 没用户 → 没开发者 → 没插件"的死循环。uTools 用动态指令部分实现了这一点，WorkBuddy 的 Markdown Skill 做得更彻底。

L0 的安全性是结构性的而非靠约束换来的：**它不执行任意代码**。声明式扩展由 `protocol/declarative.ts` 定义 schema，宿主按固定语义解释执行，能力上限由解释器决定，用户写不出解释器不支持的行为。因此 L0 无需任何审核，也不需要沙箱。

设计约束：L0 的 schema 必须保持**声明式，不得演化成图灵完备的 DSL**。一旦加入条件分支、循环、任意表达式求值，它就退化成一门需要沙箱的语言，L0 的全部安全优势随之消失。需要这类能力的场景应当引导用户升级到 L1。
...
### 7.6 能力声明向 MCP 对齐（仅协议预留）

uTools 已用 `tools` 字段以 JSON Schema 向 AI Agent 暴露插件能力，WorkBuddy 直接采用 MCP 标准。二者指向同一个杠杆：**一份能力声明，既供 launcher 检索调用，也供 AI Agent 调用**，不必维护两套。

因此 `protocol/capability.ts` 中能力声明的结构与 MCP 的 tool schema 对齐——同样的 `name` / `description` / `inputSchema`（JSON Schema）三元组。

**本版只做结构对齐，不实现任何 MCP 相关代码。** 现在对齐的成本几乎为零，而协议一旦发布就改不动；将来若要接入 MCP 生态，只需增加一个适配器而非重做协议。对面向全球的产品这条尤其划算：MCP 是国际标准，海外开发者已在产出 MCP server，兼容它等于白捡一批现成能力；自定义协议则只能自己玩。

需要明确的边界：**AI Agent 编排、多 Agent 并行、知识库等属于另一个产品形态，不在本设计范围内。** 课题全篇未提 AI，flow 的定位是 launcher 工具箱。此处只做协议层预留，不引入任何 AI 运行时。
...
调研结论（§2.4）：Raycast 明确只支持美式英语，uTools 无 i18n 字段。**插件生态的国际化没有成熟参照物**，以下每条都需自行设计。
...
课题把"数据安全，防止应用数据被盗取，防止恶意插件攻击服务器"标为 ⭐️⭐️⭐️⭐️⭐️。分层插件模型（§7）是主体方案，另需四条补充约束：
...
5. **L0 解释器不得图灵完备**（§7.3）。声明式 schema 一旦加入条件、循环或任意表达式求值，就退化成需要沙箱的语言，L0 免审的前提随之失效
...
10. 实现 L0 声明式运行时与创建界面
11. 实现动态指令与动态结果推送
12. 落地数据层与同步接口形状（含同步元字段）
13. 实现 L2 运行时与特权内置工具
...
建议在 pc 进入迁移路径第 12 步之前，单独为 `backend/` 走一次设计流程。
